<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * TimeShift Lite (local_timeshift)
 *
 * @package     local_timeshift
 * @copyright   2026 EduPlugins Studio
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['action_changeavailability'] = 'Cambiar Disponibilidad';
$string['action_deleteactivities'] = 'Eliminar';
$string['action_findreplace'] = 'Buscar y Reemplazar en Nombres';
$string['action_setallowfromdate'] = 'Establecer Fecha de Apertura';
$string['action_setcutoffdate'] = 'Establecer Fecha Límite';
$string['action_setduedate'] = 'Establecer Fecha de Entrega';
$string['action_setrestrictions'] = 'Establecer Restricciones';
$string['action_shift_dates_all'] = 'Esta acción desplazará las fechas de todas las actividades.';
$string['action_shift_dates_selected'] = 'Esta acción desplazará las fechas de {$a} actividades.';
$string['action_shiftdates'] = 'Desplazar Fechas';
$string['actionsforselected'] = 'Acciones para seleccionados';
$string['activitiesselected'] = '{$a} actividades seleccionadas';
$string['activitiesselected_plural'] = ' actividades seleccionadas';
$string['activitiesselected_singular'] = ' actividad seleccionada';
$string['activity'] = 'Actividad';
$string['activityname'] = 'Nombre de la Actividad';
$string['addtodates'] = 'Sumar a las fechas';
$string['ajaxerror'] = 'Error HTTP de AJAX';
$string['allowfromdate'] = 'Permitir Entregas / Apertura';
$string['allstatuses'] = 'Todos los estados';
$string['alltypes'] = 'Todos los tipos';
$string['apply'] = 'Aplicar';
$string['applyreplace'] = 'Aplicar Reemplazo';
$string['back'] = 'Volver';
$string['bulkshiftall'] = 'Desplazamiento Masivo (Todos)';
$string['buy_pro'] = '¡Adquiere la versión Pro aquí!';
$string['cancel'] = 'Cancelar';
$string['clearselection'] = 'Limpiar Selección';
$string['confirmdiscard'] = 'Confirmar descarte';
$string['currentdate'] = 'Fecha Actual:';
$string['cutoffdate'] = 'Fecha Límite';
$string['days'] = 'Días';
$string['direction'] = 'Dirección';
$string['discard'] = 'Descartar';
$string['discardchangeswarning'] = '¿Estás seguro de que deseas descartar todos los cambios no guardados?';
$string['domore'] = 'Descubrir Pro';
$string['duedate'] = 'Fecha de Entrega / Cierre';
$string['error'] = 'Error';
$string['errorupdate'] = 'Error al actualizar los registros en la base de datos.';
$string['example'] = 'Ejemplo:';
$string['find'] = 'Buscar';
$string['hidden'] = 'Oculto';
$string['lite_installed'] = 'Instalado exitosamente. ¡Ahorra tiempo gestionando las actividades de tu curso!';
$string['modal_delete_cannot_undo'] = 'Esta acción no se puede deshacer.';
$string['modal_delete_confirm'] = 'Sí, marcar para eliminación';
$string['modal_delete_title'] = 'Eliminar Actividades';
$string['modal_delete_warning'] = '¿Está seguro de que desea eliminar las actividades seleccionadas? Esto las eliminará permanentemente del curso y borrará todas las calificaciones y entregas de estudiantes asociadas.';
$string['modal_shift_selected_warning'] = 'Esta acción desplazará las fechas de las <strong>{$a} actividades seleccionadas</strong>.';
$string['modal_shift_warning'] = 'Esta acción desplazará las fechas de <strong>todas las {$a} actividades</strong>.';
$string['modulename'] = 'Módulo';
$string['months'] = 'Meses';
$string['newallowfromdate'] = 'Nueva Fecha de Apertura';
$string['newavailability'] = 'Nueva Disponibilidad';
$string['newcutoffdate'] = 'Nueva Fecha Límite';
$string['newdate'] = 'Nueva Fecha:';
$string['newduedate'] = 'Nueva Fecha de Entrega';
$string['opendate'] = 'Fecha de Apertura';
$string['pagedescription'] = 'Timeshift';
$string['pagedescription_help'] = 'Gestiona las fechas de apertura y cierre de las actividades del curso.<br><br>Utiliza la búsqueda para encontrar elementos específicos.';
$string['pagetitle'] = 'Timeshift';
$string['pending_deletion'] = 'Eliminación Pendiente';
$string['pluginname'] = 'Timeshift';
$string['pluginname_help'] = 'Gestiona las fechas del curso';
$string['previewchanges'] = 'Previsualizar Cambios';
$string['privacy:metadata'] = 'El plugin Timeshift no almacena datos personales.';
$string['pro_description'] = 'Desbloquea funciones avanzadas para gestionar las actividades de tu curso de manera más eficiente.';
$string['pro_feature_availability_desc'] = 'Cambia el estado de visibilidad, modo sigiloso, o oculta docenas de actividades rápidamente.';
$string['pro_feature_availability_title'] = 'Gestión de Disponibilidad';
$string['pro_feature_deletion_desc'] = 'Selecciona y elimina actividades masivamente con confirmación de seguridad para prevenir la pérdida accidental de datos.';
$string['pro_feature_deletion_title'] = 'Eliminación Segura';
$string['pro_feature_dragdrop_desc'] = 'Reordena las actividades dentro de tus secciones y temas usando una moderna interfaz de arrastrar y soltar.';
$string['pro_feature_dragdrop_title'] = 'Arrastrar y Soltar';
$string['pro_feature_filtering_desc'] = 'Encuentra las actividades exactas que deseas modificar en segundos filtrando por tipo o estado.';
$string['pro_feature_filtering_title'] = 'Filtrado Potente';
$string['pro_feature_findreplace_desc'] = 'Encuentra y modifica texto en los nombres de todas las actividades. Perfecto para actualizar años ("Examen 2026" a "Examen 2027").';
$string['pro_feature_findreplace_title'] = 'Buscar y Reemplazar';
$string['pro_feature_restrictions_desc'] = 'Asigna o elimina restricciones de acceso de forma masiva con un menú centralizado e intuitivo.';
$string['pro_feature_restrictions_title'] = 'Control de Restricciones';
$string['pro_feature_shift_desc'] = 'Desplaza las fechas de inicio, entrega y cierre de todas tus actividades o selecciones específicas hacia adelante o hacia atrás en segundos.';
$string['pro_feature_shift_title'] = 'Desplazamiento Masivo';
$string['pro_subtitle'] = 'Olvídate de editar actividad por actividad. TimeShift te ahorra horas de configuración.';
$string['pro_title'] = 'Actualiza a Timeshift Pro';
$string['pro_title_part1'] = 'Diseñado para la';
$string['pro_title_part2'] = 'Productividad';
$string['replacementtext'] = 'Texto de reemplazo...';
$string['replacewith'] = 'Reemplazar con';
$string['restrictions'] = 'Restricciones';
$string['savechanges'] = 'Guardar Cambios';
$string['saving'] = 'Guardando...';
$string['searchbyname'] = 'Buscar por nombre';
$string['section'] = 'Sección / Tema';
$string['selectwhattoshift'] = 'Seleccionar qué desplazar';
$string['shiftby'] = 'Desplazar por';
$string['shiftdays'] = 'Añadir/Restar Días';
$string['shiftmodaltitle'] = 'Desplazamiento Masivo de Fechas';
$string['shiftmodaltitle_help'] = 'Utilice esta herramienta para adelantar o atrasar todas las fechas de entrega y apertura por un número específico de días. Esto es muy útil cuando se reutiliza un curso de un semestre o año anterior.<br><br><b>Nota:</b> Si una actividad no tiene una fecha configurada, esta acción no se aplicará a ella.';
$string['status'] = 'Estado';
$string['stealth'] = 'Sigiloso';
$string['subtractfromdates'] = 'Restar de las fechas';
$string['success'] = 'Éxito';
$string['successsaved'] = 'Cambios guardados con éxito.';
$string['texttofind'] = 'Texto a buscar...';
$string['timeshift:manage'] = 'Gestionar Timeshift';
$string['totalactivities'] = 'Total de actividades:';
$string['type'] = 'Tipo';
$string['upgrade_button'] = 'Actualizar a Pro';
$string['visible'] = 'Visible';
$string['weeks'] = 'Semanas';
