...
class availability_form extends \core_form\dynamic_form {
    public function definition() {
        ...
        if (!$cmid && isset($_GET['cmid'])) $cmid = (int)$_GET['cmid'];
        if (!$cmid && isset($_POST['cmid'])) $cmid = (int)$_POST['cmid'];
        
        $courseid = $this->optional_param('courseid', 0, PARAM_INT);
        if (!$courseid && isset($_GET['courseid'])) $courseid = (int)$_GET['courseid'];
        if (!$courseid && isset($_POST['courseid'])) $courseid = (int)$_POST['courseid'];
